...
@router.get(
    "/{run_id}/report", response_model=ReportOut, response_model_exclude_none=True
)
...
async def regenerate_report(
    ...
    if run.status not in {"completed", "cancelled", "failed"}:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "Report regeneration requires a terminal run"
        )
    report_repo = ReportRepository(db)
    report = await report_repo.get_by_run(run_id)
    if report is not None:
        await report_repo.update(
            report, pdf_path=None, generation_status="generating", generation_error=None
        )
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status.HTTP_500_INTERNAL_SERVER_ERROR, f"Report regeneration failed: {exc}"
        ) from exc
...
@router.get(
    "/{run_id}/report/pdf",
    responses={
        200: {"content": {"application/pdf": {}}, "description": "QA report PDF"}
    },
...
async def get_report_pdf(
    ...
    if report is None or not report.pdf_path:
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR,
                f"Report PDF generation failed: {exc}",
            ) from exc
    if not report.pdf_path:
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE,
            "Report generation is still in progress, try again later",
        )
