...
from app.models import AgentPositionTrail, Defect, GameEvent
...
class RunCompareService:
    ...
    async def compare(self, run_a_id: UUID, run_b_id: UUID) -> dict[str, Any]:
        ...
        if run_a is None or run_b is None:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, "One or both runs were not found"
            )
        same_map = (
            run_a.wad_file_id == run_b.wad_file_id and run_a.map_name == run_b.map_name
        )
        if not same_map:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Runs must reference the same WAD and map"
            )
        ...
        return {
            ...
            "resource_delta": {
                ...
                "final_ammo_delta": _none_safe_delta(
                    _last_ammo(events_b), _last_ammo(events_a)
                ),
            },
        }
    ...
    async def _defects(self, run_id: UUID) -> dict[str, dict[str, Any]]:
        ...
        for defect in result.scalars().all():
            key = (
                defect.fingerprint
                or f"{defect.defect_type}:{defect.title}:{defect.detected_at_tick}"
            )
            defects[key] = {
    ...
    async def _movement_coverage(self, run_id: UUID) -> float:
        ...
        cells = {
            (round(position.x / CELL_SIZE), round(position.y / CELL_SIZE))
...
def _last_ammo(events: list[GameEvent]) -> int | None:
    ...
    event = events[-1]
    return (
        event.ammo_bullets + event.ammo_shells + event.ammo_rockets + event.ammo_cells
    )
