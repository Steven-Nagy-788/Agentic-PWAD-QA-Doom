...
def render_agent_prompt(
    ...
    spawned_enemies = int(
        skill_summary.get("thing_count_enemies", analysis.thing_count_enemies) or 0
    )
    ...
    values: dict[str, Any] = {
        ...
        "estimated_difficulty": skill_summary.get("estimated_difficulty")
        or analysis.estimated_difficulty
        or "unknown",
        "thing_count_enemies": spawned_enemies,
        "raw_thing_count_enemies": raw_enemies,
        "enemy_breakdown_summary": json.dumps(
            skill_summary.get("enemy_breakdown") or {}, ensure_ascii=True
        ),
        "raw_enemy_breakdown_summary": json.dumps(
            analysis.enemy_breakdown, ensure_ascii=True
        ),
        "hitscanner_percent": str(
            skill_summary.get("hitscanner_percent", analysis.hitscanner_percent)
            or "unknown"
        ),
        "health_ratio": str(
            skill_summary.get("health_ratio", analysis.health_ratio) or "unknown"
        ),
        "ammo_ratio": str(
            skill_summary.get("ammo_ratio", analysis.ammo_ratio) or "unknown"
        ),
        ...
        "total_health_pickup_pts": skill_summary.get(
            "total_health_pickup_pts", analysis.total_health_pickup_pts
        )
        or 0,
