...
def _update_lockstep_after_action(
    ...
    lockstep_state["total_decision_count"] = (
        int(lockstep_state.get("total_decision_count") or 0) + 1
    )
    ...
    lockstep_state["action_signature_counts"] = dict(
        list(signature_counts.items())[-50:]
    )
    _record_attempted_interaction(tool, params, summary, lockstep_state)

    if tool == "explore" and stop_reason == "max_tics":
        lockstep_state["low_value_explore_cumulative"] = (
            int(lockstep_state.get("low_value_explore_cumulative") or 0) + 1
        )
    if stop_reason == "invalid_params":
        lockstep_state["blocked_decision_count"] = (
            int(lockstep_state.get("blocked_decision_count") or 0) + 1
        )
    ...
    if tool == "move_to" and object_id is not None:
        ...
        if stop_reason == "arrived":
            ...
            if key not in completed:
                lockstep_state["progress_score"] = (
                    int(lockstep_state.get("progress_score") or 0) + 2
                )
                lockstep_state["meaningful_progress_events"] = (
                    int(lockstep_state.get("meaningful_progress_events") or 0) + 1
                )
            completed[key] = {
    ...
    if tool in COMBAT_TOOLS and stop_reason in _RESOURCE_STOP_REASONS:
        ...
        _append_warning(
            lockstep_state, _weapon_resource_warning(object_id, mcp_call, stop_reason)
        )
    ...
    if stop_reason in {"target_killed", "shots_complete"} and (
        ...
        lockstep_state["progress_score"] = (
            int(lockstep_state.get("progress_score") or 0) + 3
        )
        lockstep_state["meaningful_progress_events"] = (
            int(lockstep_state.get("meaningful_progress_events") or 0) + 1
        )
        ...
        lockstep_state["progress_score"] = (
            int(lockstep_state.get("progress_score") or 0) + 1
        )
...
def _update_no_progress_stall(
    state_after: dict[str, Any] | None, lockstep_state: LockstepState
) -> None:
    ...
    if signature == lockstep_state.get("last_progress_signature"):
        lockstep_state["no_progress_polls"] = (
            int(lockstep_state.get("no_progress_polls") or 0) + 1
        )
    else:
...
def _weapon_resource_warning(
    object_id: Any, mcp_call: dict[str, Any], stop_reason: str
) -> str:
    state = _weapon_state_from_call(mcp_call)
...
def _lockstep_progress_metrics(lockstep_state: LockstepState) -> dict[str, Any]:
    ...
    return {
        ...
        "meaningful_progress_events": int(
            lockstep_state.get("meaningful_progress_events") or 0
        ),
        ...
        "weapon_resource_failure_count": len(
            lockstep_state.get("weapon_resource_failures") or {}
        ),
        "blocked_decision_count": int(
            lockstep_state.get("blocked_decision_count") or 0
        ),
        "low_value_explore_count": int(
            lockstep_state.get("low_value_explore_cumulative") or 0
        ),
        "visited_cells_count": visited_count,
        "total_map_cells_estimate": total_cells,
        "coverage_percent": round(visited_count / max(int(total_cells), 1) * 100, 1)
        if total_cells
        else None,
        ...
        "consecutive_no_progress_decisions": int(
            lockstep_state.get("no_progress_polls") or 0
        ),
    }


def _lockstep_quality_flags(
    lockstep_state: LockstepState, recording_metadata: dict[str, Any]
) -> dict[str, Any]:
    warnings = list(lockstep_state.get("quality_warnings") or [])
