...
class McpDoomClient:
    ...
    async def __aenter__(self) -> "McpDoomClient":
        try:
            ...
            raise RuntimeError(
                "fastmcp is not installed in the backend environment"
            ) from exc
        ...
        raise McpStartupError(
            "MCP SSE connection failed after startup retries"
        ) from last_error
    ...
    async def call_tool(self, name: str, params: dict[str, Any] | None = None) -> Any:
        ...
        try:
            return await asyncio.wait_for(
                self._client.call_tool(name, params or {}), timeout=timeout
            )
        except asyncio.TimeoutError as exc:
            raise McpToolTimeoutError(
                f"MCP tool {name} timed out after {timeout:g}s"
            ) from exc
    ...
    async def start_game(
        ...
        raise McpStartupError(
            f"MCP start_game failed after startup retries{detail}"
        ) from last_error

    async def get_state(self) -> tuple[dict[str, Any], bytes | None]:
        result = await self.call_tool(
            "get_state", {"include_sectors": False, "include_depth": True}
        )
...
async def probe_mcp_sse_url(
    url: str | None = None, timeout_seconds: float | None = None
) -> dict[str, Any]:
    ...
    timeout = (
        ...
        else settings.mcp_probe_timeout_seconds
    )
    started = time.monotonic()
    try:
        async with httpx.AsyncClient(
            timeout=max(0.1, timeout), follow_redirects=True
        ) as client:
...
def normalize_mcp_state(result: Any) -> tuple[dict[str, Any], bytes | None]:
    ...
    for item in items:
        ...
        mime_type = (
            ...
            or getattr(item, "format", "")
        )
        if data is not None and (
            "image" in mime_type or mime_type in {"png", "jpeg", "jpg"}
        ):
            ...
            state = item
        elif hasattr(item, "structured_content") and isinstance(
            item.structured_content, dict
        ):
