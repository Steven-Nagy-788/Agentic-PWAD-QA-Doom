...
This chapter presents the testing methodology, validation results, and quality assurance practices applied to the Agentic PWAD QA system. The system comprises three services — a FastAPI backend, a FastMCP/ViZDoom game integration layer, and a Next.js frontend — each tested according to its specific risk profile and interaction patterns.

## 7.1 Testing Strategy

The testing strategy follows a multi-layered approach aligned with the testing pyramid. Automated unit and integration tests form the foundation, supplemented by manual usability evaluation and performance profiling. Testing was guided by three objectives: correctness of the lockstep agent loop that drives autonomous Doom gameplay, robustness of API endpoints and data pipelines under realistic conditions, and usability of the real-time monitoring interface.
...
### 7.2.1 Backend Service

The Backend received the most extensive testing, with **329 tests** covering guard logic, prompt sanitization, MCP state normalization, tool request validation, configuration validation, and run loop decision handling.
...
### 7.2.2 MCP-Doom Service

The MCP-Doom service contributed **66 unit tests** covering edge cases in spatial navigation, breadcrumb memory limits, key tracking across multiple objects, and combat constant completeness. These tests verify the pure-logic components of the navigation and state systems.

The relatively low line coverage (28%) is explained by the heavy dependency on the ViZDoom runtime for game execution logic, which requires OpenGL and SDL libraries. This code path is exercised by integration tests executed separately under a display server.

### 7.2.3 Frontend

The Frontend received **43 unit tests** covering the API utility functions (WebSocket URL construction, asset URL normalization, date formatting), the AsciiGrid component's header-grid separation logic and legend rendering, and the DefectBadge component's severity-based color tone mapping. These tests confirm that visual indicators render correctly and that data transformation utilities handle edge cases such as null inputs and empty strings.
...
| ID | Description | Input | Expected Outcome | Status |
|---|---|---|---|---|
| TC01 | Guard forces explore after consecutive get_state calls | `consecutive_get_state=2`, tool=`get_state` | Tool overridden to `explore` with 180-degree turn | Pass |
| TC02 | Guard allows finish when thresholds are met | `coverage_percent=95`, `player_kills=10` | Tool remains `finish`, no guard override | Pass |
| TC03 | Guard blocks premature finish when conditions unmet | `coverage_percent=30`, `player_kills=2` | Tool overridden to `explore` | Pass |
| TC04 | Tool validation rejects unsupported tool names | tool=`"fly_to_moon"` | Validation error returned | Pass |
| TC05 | Tool validation requires object_id for combat tools | tool=`"aim_and_shoot"`, missing `object_id` | Validation error for missing parameter | Pass |
| TC06 | MCP state normalization handles plain dict input | `{"game_variables": {"x": 1}}` | Dict returned as state, no screenshot | Pass |
| TC07 | MCP state normalization extracts base64 image data | Image object with base64 payload | Decoded bytes returned as screenshot | Pass |
| TC08 | Prompt sanitizer replaces curly braces in user content | `"{hello} world"` | `"(hello) world"` | Pass |
| TC09 | DefectBadge applies correct color tone by count | counts 0, 1, 5 | Emerald (0), Amber (1-2), Red (3+) | Pass |
| TC10 | Frontend assetUrl returns undefined for null input | `assetUrl(null)` | `undefined` | Pass |
| TC11 | NavigationMemory tracks visited grid cells | Updates at three coordinates | `cells_explored >= 3` | Pass |
| TC12 | Configuration validator rejects invalid tick settings | `MAX_RUN_TICKS=100`, `DEFAULT_RUN_TICKS=500` | `ValueError` raised | Pass |
| TC13 | Run loop normalizes legacy outcome values | `"agent_died"`, `"softlock"` | Maps to `"player_died"`, `"inconclusive_agent_stall"` | Pass |
| TC14 | WebSocket URL constructed correctly from local origin | `origin="http://localhost:3000"` | URL uses `ws://localhost:8000` | Pass |
| TC15 | MCP-Doom navigation cell function handles negative coords | `_cell(-128, -128)` | Returns `(-1, -1)` | Pass |

All 438 automated tests pass in the CI pipeline. No tests are marked as expected failures.

---

## 7.5 Usability and Performance Testing
...
|---|---|---|---|
| #12 | CORS policy blocking frontend API requests in development | High | Updated `Settings._derive_and_validate` to append localhost origins when `APP_ENV=development` or `DEBUG=true` |
| #15 | Database connection timeout under concurrent run loads | High | Implemented `asyncpg` connection pooling with health checks before run initialization |
| #19 | Missing file size validation on WAD upload endpoint | Medium | Added `MAX_WAD_UPLOAD_BYTES` configuration with pydantic validator and pre-write size check |
| #22 | Unresponsive grid layout on mobile viewport for live map | Low | Replaced fixed-width container with responsive flexbox and added `preserveAspectRatio` to SVG viewBox |
| #28 | LLM rate limiter not respecting per-model limits | High | Added `asyncio.Semaphore` with sliding window rate limiter tracking recent API call timestamps |
| #33 | WebSocket reconnect loop flooding server on network interruption | Medium | Added exponential backoff to `useRunStream` hook; capped maximum reconnect attempts at 5 |
| #37 | PDF report rendering fails with special characters in WAD filenames | Medium | Added prompt value sanitization to escape curly braces before Jinja2 template rendering |
| #41 | MCP tool timeout not propagating error context to run failure records | Low | Wrapped `McpToolTimeoutError` with structured failure fields for improved debugging |

**Resolution Pattern:** High-severity infrastructure and configuration issues (CORS, database pooling, LLM rate limiting) were resolved in the first third of the development timeline, while lower-severity UI and edge-case issues were addressed in later iterations. No critical (severity 1) bugs remained unresolved at the time of deployment. The average resolution time for high-severity issues was 13.5 hours; medium and low severity issues averaged 4.3 hours.
...
Coverage remains uneven across services: the Backend achieves 57% line coverage, the Frontend 61%, while MCP-Doom unit coverage sits at 28% due to the heavy ViZDoom dependency. The 13 skipped integration tests in MCP-Doom require the ViZDoom runtime and are executed separately in CI under Xvfb. Future work should focus on increasing coverage of the Backend's `run_loop.py` and `report_service.py`, which represent the most complex and business-critical modules in the system.

No critical defects remain open. The system is validated for demonstration and deployment.
